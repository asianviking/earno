...
export const send = {
  description: 'Send USDT0 cross-chain via LayerZero (zero slippage)',
  args: z.object({
    amount: z.string().describe('Amount of USDT to send (e.g. "100")'),
  }),
  options: z.object({
    to: z
      .string()
    ...
    sender: z
      .string()
      .optional()
      .describe('Sender wallet address (defaults to --to)'),
    rpc: z
      .string()
      .optional()
  ...
  examples: [
    {
      ...
      description: 'Send 100 USDT0 from Arbitrum to Berachain',
    },
    {
      args: { amount: '50' },
      options: {
      ...
      description: 'Send 50 USDT from Ethereum to Berachain (locks USDT, mints USDT0)',
    },
  ],
  async run(c: any) {
    const { amount } = c.args
    ...
    let to: string
    try {
      to = getAddress(c.options.to as string)
    } catch {
      return c.error({ code: 'INVALID_ADDRESS', message: 'Invalid --to address', retryable: true })
    }
    let sender: string
    try {
      sender = getAddress((c.options.sender ?? to) as string)
    } catch {
    ...
    if (srcChainKey === dstChainKey) {
      return c.error({
        code: 'SAME_CHAIN',
        message: 'Source and destination chains must be different. Use a regular transfer for same-chain sends.',
        retryable: true,
      })
    }
    ...
    // Approve token → OFT (required on all chains since token and OFT are separate)
    calls.push({
    ...
    // The send() call on the OFT contract
    calls.push({
      label: `Send ${amount} ${tokenLabel} → ${dstChainKey}`,
    ...
    const req = {
      title: `Send ${amount} ${tokenLabel} → ${dstChainKey}`,
      chainId: srcChain.id,
      rpcUrl,
      sender: sender as `0x${string}`,
      ...
      intent: {
        ...
        display: {
          kind: 'transfer',
          strategy: `USDT0 via LayerZero (zero slippage)`,
        },
      },
      calls,
